...
1. **Market Orders Only**: Simplified implementation focusing on immediate execution
2. **Mock DEX Router**: Simulates Raydium/Meteora for demo purposes
3. **Event-Driven**: EventEmitter pattern for decoupled status updates
4. **Concurrent Processing**: BullMQ with concurrency:10 for scalability
5. **Dual Persistence**: PostgreSQL for state, MongoDB for audit logs

### Future Extensions

**Limit Orders**: Store target price, run background checker to poll quotes and trigger when threshold crossed.

**Sniper Orders**: Subscribe to token launch events via blockchain indexer WebSocket, auto-trigger order creation and route through existing pipeline.
...
- [Frontend README](frontend/README.md) - UI usage guide
- [Postman Collection](backend/postman/order-execution.collection.json) - API testing

## 🤝 Contributing

This is a demo project showcasing order execution architecture. Feel free to fork and extend!
